...
export function ConnectionDocs() {
  ...
  return <div className="dw-page dw-docs-page">
    ...
    {tab === 'mcp' && <div className="dw-doc-layout"><div className="dw-doc-main">
      ...
      <section className="dw-doc-section"><div className="dw-section-heading"><div><span className="dw-section-number">01</span><h2>在 WorkBuddy 中连接</h2></div></div><p>添加 MCP Server 后，浏览器将跳转到企业身份登录。Access Token 只通过 HTTPS Authorization Header 传输。</p><CodeBlock code={workBuddy} /></section>
    ...
    {tab === 'sdk' && <div className="dw-doc-single">
      <section className="dw-doc-section"><div className="dw-section-heading"><div><span className="dw-section-number">01</span><h2>Python SDK</h2></div><span className="dw-doc-note">批准语言</span></div><CodeBlock code="pip install openconnector" /><CodeBlock code={`import os\nfrom openconnector import Client\n\nclient = Client(\n    endpoint="${endpoint}",\n    access_token=os.environ["ACCESS_TOKEN"],\n)\nconnections = client.connections.list()\nprint([item.name for item in connections])`} /></section>
      <section className="dw-doc-section"><h2>TypeScript SDK</h2><CodeBlock code="pnpm add @openconnector/sdk" /><CodeBlock code={`import { OpenConnector } from "@openconnector/sdk";\n\nconst client = new OpenConnector({\n  endpoint: "${endpoint}",\n  getAccessToken: () => authSession.accessToken(),\n});\nconst connections = await client.connections.list();`} /></section>
    </div>}
    <ReadOnlyTester />

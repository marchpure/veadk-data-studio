...
class KnowledgeResource(Base):
    ...
    last_indexed_at: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=False), nullable=True)
    provider_error: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    retrieval_debug_uri: Mapped[str | None] = mapped_column(Text, nullable=True)

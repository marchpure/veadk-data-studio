...
import { DemoBadge, EmptyState, ErrorState, PermissionState, ScoreBar, StatusPill } from '../components/modelingUi'
import { useDataModelingStore } from '../store/useDataModelingStore'
import type { HomeDemoMode, SemanticModel } from '../types'

const modes: HomeDemoMode[] = ['ready', 'loading', 'empty', 'error', 'permission']
...
export default function DataModelsHomePage() {
  ...
  const resetDemo = useDataModelingStore(state => state.resetDemo)
      ...
      <div className="mx-auto flex max-w-[1480px] flex-col gap-4">
        <header className="flex flex-col gap-3 border-b border-[#2a2a2a] pb-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="secondary" onClick={resetDemo}><RefreshCcw className="h-4 w-4" /> Reset Demo</Button>
        ...
        <section className="rounded-lg border border-[#2a2a2a] bg-[#1f1f1f]">
          <div className="flex flex-col gap-3 border-b border-[#2a2a2a] p-3 lg:flex-row lg:items-center">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <div className="flex items-center gap-1 rounded-md border border-[#333] bg-[#151515] p-1">
                <SlidersHorizontal className="ml-2 h-4 w-4 text-[#777]" />
                {modes.map(mode => (
                  <button
                    key={mode}
                    onClick={() => setHomeMode(mode)}
                    className={`rounded px-2 py-1 text-xs focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring ${homeMode === mode ? 'bg-brand-orange text-white' : 'text-[#bdbdbd] hover:bg-[#262626]'}`}
                  >
                    {mode}
                  </button>
                ))}
          ...
          <div className="p-3">
            ...
            {!homeLoading && !homeError && filteredModels.length === 0 && (
              <EmptyState
                ...
                body="This mock empty state covers first-run or filtered workspaces. Generate a model from Oracle SALES to continue the demo."

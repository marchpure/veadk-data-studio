...
def build_matrix_rows() -> list[MatrixRow]:
    ...
    rows = [
        ...
        MatrixRow(
            source_type="legacy_dataset_parquet_json",
            provider_adapter="Legacy file dataset upload / server/routers/file_upload.py",
            availability="available",
            auth_config_contract="none; tenant/user captured from authenticated upload",
            browse_select_import_contract="POST /api/datasets/upload-files and upload-from-url support .parquet and .json as DuckDB-backed datasets",
            snapshot_raw_artifact="Dataset/File storage only; no SourceResource or SourceSnapshot contract for local direct upload",
            parser_profile_contract="DataFrameFileService schema/profile through dataset flow",
            modeling_mode="tabular_projection",
            fixture="legacy dataset/file tests; no governed SourceResource fixture",
            api_journey="dataset upload -> /api/datasources -> schema/profile; no source-resources snapshots/lineage",
            ui_journey="client/src/pages/Databases.tsx legacy upload accepts .parquet/.json",
            lineage_evidence="Dataset/File checksum and schema_cache; source lineage/evidence endpoints missing",
            retry_revoke="Dataset delete/update only; no source retry/reindex lifecycle",
            final_status="blocked",
            blocker_reason="Parquet/JSON are real dataset flows but not yet unified into the governed Source Contract for local SourceResource upload",
        ),
        MatrixRow(
            source_type="web_url",
        ...
        MatrixRow(
            source_type="sql_pg_mysql_sqlite_oracle",
            ...
            final_status="beta",
            blocker_reason="Structured model loop exists for PG/MySQL/SQLite/Oracle; SQL Server is connected but not analyzer-supported",
        ),
        MatrixRow(
            source_type="sqlserver_mssql",
            provider_adapter="ConnectionService + DatabaseOperationsService.get_sql_schema_async",
            availability="available",
            auth_config_contract="connection form and encrypted Connection credentials",
            browse_select_import_contract="Database connection form and schema refresh",
            snapshot_raw_artifact="No SourceAnalyzer database_* snapshot contract",
            parser_profile_contract="Schema/query support exists; Source Understanding analyzer excludes mssql",
            modeling_mode="blocked_structured",
            fixture="connection/query tests only; no Source Understanding semantic draft fixture",
            api_journey="/connections and /datasources schema work; /understanding/analyze rejects unsupported type",
            ui_journey="Databases form exists; Data Modeling legacy fallback does not include mssql",
            lineage_evidence="Connection schema_cache only",
            retry_revoke="refresh schema/delete connection",
            final_status="blocked",
            blocker_reason="MSSQL is a real connection type but not yet in SourceAnalyzer/Data Modeling semantic draft support",
        ),
        MatrixRow(
            source_type="mongo",
...
def render_markdown() -> str:
    ...
    lines = [
        ...
        "|---|---|---|",
        "| Structured Data Modeler | `sql_pg_mysql_sqlite_oracle`, `databricks` | Schema/profile, Source Understanding, semantic draft, publish/reload, and MCP metric preview exist for the listed beta rows. |",
        "| Tabular Projection Modeler | local CSV/Excel, Feishu Sheets/Base, TOS tabular objects | Projection manifests, raw snapshots, field/profile evidence, and projected datasets exist; review/confirmation remains beta. |",
        ...
        "| Blocked projection/modeling | Parquet/JSON local SourceResource upload, MSSQL, MongoDB, DynamoDB | Current query or dataset capability exists, but governed source snapshot/projection/modeling handoff is incomplete. |",
        "",
    ...
    lines.extend(
        [
            ...
            "## Current Gap Report",
            "",
            "- Local Parquet/JSON are real legacy dataset uploads and TOS object parsers, but local governed `SourceResource` upload currently accepts only CSV, XLSX/XLSM, PDF, DOCX, and PPTX.",
            "- SQL Server, MongoDB, and DynamoDB can be represented as connections/query sources, but they do not yet satisfy the P0 source snapshot/projection/modeling handoff contract.",

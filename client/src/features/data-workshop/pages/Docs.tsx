...
export function ConnectionDocs() {
  ...
  return <div className="dw-page dw-docs-page">
    <div className="dw-page-heading dw-heading-row"><div><span className="dw-eyebrow">连接 / 文档</span><h1>使用连接能力</h1><p>通过 OAuth / JWT，以 MCP、HTTP API 或 SDK 调用当前身份获准的能力。</p></div><span className={`dw-health ${status.status}`}><HeartPulse size={16} />{status.backend_mode === 'TEST' ? '测试后端' : status.status === 'healthy' ? '服务正常' : status.status === 'degraded' ? '服务降级' : '服务不可用'}</span></div>
    {status.identity_status !== 'ready' && <div className="dw-identity-warning" role="alert"><AlertCircle size={18} /><div><strong>{status.identity_status === 'unverified' ? 'Identity / JWKS 尚未验证' : 'Identity / JWKS 尚未配置'}</strong><span>{status.identity_status === 'unverified' ? '配置已保存，但上游未提供健康证明。请完成真实用户登录验证后重试。' : '请在 OpenConnector Console 配置企业 Identity Provider 与 JWKS，完成后重试。'}</span></div><div className="dw-button-row"><a className="dw-button dw-button-secondary" href="/connections/access/identity">打开身份配置</a><button className="dw-button dw-button-secondary" onClick={() => void load()}>重试</button></div></div>}
    ...
    </div><aside className="dw-doc-aside"><div><strong>身份状态</strong><span className={`dw-status ${config.identity.status === 'ready' ? 'ready' : 'error'}`}>{config.identity.status === 'ready' ? '已配置' : '需处理'}</span></div><dl><dt>UserPool</dt><dd>{config.identity.user_pool_ref || '-'}</dd><dt>Issuer</dt><dd>{config.identity.issuer || '-'}</dd><dt>Audience</dt><dd>{config.identity.audience?.join(', ') || '-'}</dd><dt>JWKS</dt><dd>{config.identity.jwks_status || '-'}</dd></dl><a href="/connections/access">查看访问权限 <ExternalLink size={14} /></a><a href="/connections/runs">查看运行记录 <ExternalLink size={14} /></a></aside></div>}
    {tab === 'http' && <div className="dw-doc-single">
      <section className="dw-doc-section"><div className="dw-section-heading"><div><span className="dw-section-number">01</span><h2>版本化 HTTP API</h2></div></div><p>用户运行时请求使用 <code>/v1/*</code>。自然人请求默认使用 OAuth Bearer Token，不在 URL 或浏览器存储中保存凭据；管理接口仅由 Studio BFF 在服务端调用。</p><CodeBlock code={`curl --request GET \\\n  '${new URL('/v1/apps', endpoint).toString()}' \\\n  --header 'Authorization: Bearer $ACCESS_TOKEN' \\\n  --header 'Accept: application/json'`} /></section>

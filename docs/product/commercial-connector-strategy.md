...
- `available` means the flow is selectable in the commercial beta, not that every readiness gate is already 10/10 production-hard. Available connectors now expose `partial` and `not_applicable` readiness gates where the current implementation is honest beta hardening, such as sitemap/page-group crawl policy, tenant-wide revoke, scheduled sync, warehouse raw-artifact non-applicability, or parser warning taxonomy. Planned connectors still expose all gates as `missing` and remain request-access/read-only.
...
- Legacy CSV/Excel/Parquet/JSON dataset upload and URL import remain available as beta dataset-only compatibility flows. They are not the commercial Files adapter contract because they do not create governed Source snapshots, context evidence, Source Detail lineage, or KnowledgeProvider status.
...
The processing API should expose the commercial stepper as structured state, not only as a free-form message. Each source resource reports typed `status`, typed `stage`, and step ids for `Capture`, `Parse`, `Detect tables`, `Normalize dataset`, `Index context`, `Generate semantic suggestions`, and `Ready` with `pending`, `running`, `succeeded`, `skipped`, or `failed` status so Add Source results, Source Detail, and modeling handoff can explain the same recovery path.
...
Connector resource picker failures should be persisted as connection health, not only returned as transient API errors. When a picker encounters reauthorization, permission, or upstream availability failures, `SourceConnection.status` and a redacted `capabilities.last_error` record the product state and recovery context so Sources list, Source Detail, and the Add Source flow can explain why browsing resources is blocked.
...
SQL and warehouse connection-backed Sources must not be treated as semantic-ready merely because a legacy dataset wrapper exists. The `SourceOverview` facade should gate relational readiness on a usable schema/profile cache: missing or empty schema/profile returns `Pending` with a refresh action, invalid schema cache returns `Failed` with a parser/profile recovery action, and only a non-empty table profile is `Ready` for relational or warehouse modeling. This keeps Data Modeling from promoting broken database connections into production semantic generation.

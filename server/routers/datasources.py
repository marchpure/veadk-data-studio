...
def _source_understanding_not_found_or_bad_request(error: ValueError) -> HTTPException:
    message = str(error)
...
@router.get(
    "/datasources/{datasource_id}/understanding",
    response_model=StandardResponse[SourceUnderstandingRead],
)
async def get_datasource_understanding(
    ...
    """Return the latest Source Understanding workspace payload for a database datasource."""
    if auth.is_viewer:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Viewers cannot access Source Understanding")
    try:
        data = await source_understanding_service.get_understanding(
            session=session,
            datasource_id=datasource_id,
            tenant_id=auth.tenant_id,
        )
        return success_response(data=data, message="Retrieved datasource understanding")
    except ValueError as error:
        raise _source_understanding_not_found_or_bad_request(error)


@router.post(
    "/datasources/{datasource_id}/understanding/analyze",
    response_model=StandardResponse[SourceUnderstandingRead],
)
async def analyze_datasource_understanding(
    datasource_id: str,
    payload: SourceAnalyzeRequest | None = None,
    auth: AuthContext = Depends(require_any_scope(Scope.DATASET_UPDATE, Scope.DATASET_UPDATE_OWN)),
    session: AsyncSession = Depends(get_async_session),
):
    """Analyze a database datasource into Source Resources, Snapshots, Evidence, and Source Skill candidates."""
    request = payload or SourceAnalyzeRequest()
    try:
        allowed = await source_understanding_service.can_update_datasource(
            session=session,
            datasource_id=datasource_id,
            tenant_id=auth.tenant_id,
            user_id=auth.user_id,
            update_all=auth.has_scope(Scope.DATASET_UPDATE),
        )
        if not allowed:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You can only analyze datasources you created",
            )
        data = await source_understanding_service.analyze_database(
            session=session,
            datasource_id=datasource_id,
            tenant_id=auth.tenant_id,
            user_id=auth.user_id,
        ...
        return success_response(data=data, message="Datasource understanding analysis completed")
...
@router.post(
    "/datasources/{datasource_id}/understanding/candidates/{candidate_id}/review",
    response_model=StandardResponse[SourceUnderstandingRead],
)
async def review_source_skill_candidate(
    ...
    """Accept, edit, or reject a Source Skill candidate. Only verified candidates can feed semantic drafts."""
    try:
        allowed = await source_understanding_service.can_update_datasource(
            session=session,
            datasource_id=datasource_id,
        ...
        if not allowed:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You can only review candidates for datasources you created",
            )
        data = await source_understanding_service.review_candidate(
            session=session,
            datasource_id=datasource_id,
        ...
        return success_response(data=data, message="Source Skill candidate reviewed")
    except HTTPException:
        raise
    except ValueError as error:
        raise _source_understanding_not_found_or_bad_request(error)
...
@router.post(
    "/datasources/{datasource_id}/understanding/semantic-model-draft",
    response_model=StandardResponse[SourceToSemanticModelResponse],
)
async def create_semantic_model_draft_from_source_understanding(
    ...
    """Create or update a Semantic Model draft from verified Source Skill candidates."""
    try:
        allowed = await source_understanding_service.can_update_datasource(
            session=session,
            datasource_id=datasource_id,
        ...
        if not allowed:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You can only create semantic drafts for datasources you created",
            )
        data = await source_understanding_service.create_or_update_semantic_model_from_verified(
            session=session,
            datasource_id=datasource_id,
            tenant_id=auth.tenant_id,
            user_id=auth.user_id,
        ...
        return success_response(data=data, message="Semantic model draft updated from verified source candidates")
    except HTTPException:
        raise
    except ValueError as error:
        raise _source_understanding_not_found_or_bad_request(error)
...
async def list_all_datasources(
    ...
    try:
        ...
        source_resources_result = await session.execute(
            ...
            .where(
                SourceResource.tenant_id == auth.tenant_id,

...
from server.services.source_connectors import ConnectorError, FeishuAdminConfigService
...
async def save_feishu_admin_config(
    ...
    return success_response(data=await FeishuAdminConfigService.status(session=session), message="Saved Feishu config")

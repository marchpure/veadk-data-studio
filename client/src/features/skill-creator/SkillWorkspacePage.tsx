...
export default function SkillWorkspacePage() {
  ...
  const load = useCallback(async (idToLoad: string) => {
    const value = await skillCreatorApi.getSession(idToLoad)
    setSession(value)
    setTarget(value.target)
    setMcp(value.mcp_refs.map(ref => ref.id))
    setKnowledge(value.knowledge_refs.map(ref => ref.id))
    setLastGoal(value.messages.filter(message => message.role === 'user').at(-1)?.content || '')
  }, [])

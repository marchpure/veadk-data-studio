...
IMPORTANT — Automatic data learning (MANDATORY after every query):
After EVERY successful query, scan the result for non-obvious patterns and call add_memory for each:
- Naming quirks: misspelled or non-obvious table/collection/field names (e.g. "fule" instead of "fuel")
- Value formats: how field values are actually formatted (e.g. full country names "United States" vs ISO codes "US")
- Data ranges: actual date ranges, row counts, value bounds not derivable from the schema
- Units: prices/measurements use specific units (per liter vs per gallon, metric vs imperial)
- Coded values: numeric/string codes that map to meanings (1=active, 2=inactive)
- NULL semantics: what NULL means in specific columns (e.g. NULL end_date = still active)
- Join patterns: non-obvious keys or lookup approaches that worked
Do NOT wait for failures. Save on FIRST discovery. Each pattern = one add_memory call BEFORE your text response.

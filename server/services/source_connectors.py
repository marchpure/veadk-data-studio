...
class FeishuAdminConfigService:
    @staticmethod
    def generated_redirect_uri() -> str:
    ...
    def hosted_config() -> dict[str, Any]:
        app_id = (os.getenv(FEISHU_HOSTED_APP_ID_ENV) or "").strip()
        app_secret = (os.getenv(FEISHU_HOSTED_APP_SECRET_ENV) or "").strip()
        redirect_uri = FeishuAdminConfigService.generated_redirect_uri()
        return {
            "app_id": app_id,
    ...
    async def save_config(
        *,
        ...
        existing = await FeishuAdminConfigService.load_config(session=session) or {}
        next_secret = (app_secret or "").strip() or str(existing.get("app_secret") or "").strip()
        if not next_secret:
            raise ConnectorError("App Secret is required for first-time Feishu configuration", code="admin_config_required", permanent=True)
        encrypted = await CryptoService.encrypt_config(
            {
                "app_id": app_id.strip(),
                "app_secret": next_secret,
    ...
    async def load_config(*, session: AsyncSession) -> dict[str, Any] | None:
        ...
        if not setting or not setting.setting_value:
            hosted = FeishuAdminConfigService.hosted_config()
            return hosted if hosted["app_id"] and hosted["app_secret"] else None
    ...
    async def status(*, session: AsyncSession, include_admin_details: bool = False) -> dict[str, Any]:
        ...
        if not config:
            return {
                ...
                "generated_redirect_uri": FeishuAdminConfigService.generated_redirect_uri(),
        ...
        if not include_admin_details:
            data.pop("secret_configured", None)
        return data

    @staticmethod
    async def validate_config(*, session: AsyncSession) -> dict[str, Any]:
        status = await FeishuAdminConfigService.status(session=session, include_admin_details=True)
        checks = {
            "configured": {
                "ok": bool(status.get("configured")),
                "message": "Feishu app credentials and redirect URI are configured."
                if status.get("configured")
                else "Feishu app credentials or redirect URI are missing.",
            },
            "secret": {
                "ok": bool(status.get("secret_configured")),
                "message": "App Secret is stored encrypted and will not be returned."
                if status.get("secret_configured")
                else "App Secret has not been configured.",
            },
            "scopes": {
                "ok": not status.get("missing_scopes"),
                "message": "Required scopes are present."
                if not status.get("missing_scopes")
                else f"Missing scopes: {', '.join(status.get('missing_scopes') or [])}",
            },
            "redirect_uri": {
                "ok": bool(status.get("redirect_uri") or status.get("generated_redirect_uri")),
                "message": f"Redirect URI: {status.get('redirect_uri') or status.get('generated_redirect_uri') or 'missing'}",
            },
        }
        return {"ok": all(check["ok"] for check in checks.values()), "status": status, "checks": checks}
...
class FeishuOAuthStateStore:
    ...
    @classmethod
    def peek(cls, state: str) -> dict[str, Any] | None:
        value = cls._states.get(state)
        if value is None:
            return None
    ...
    @classmethod
    def set_result(cls, state: str, result: dict[str, Any]) -> None:
        cls._results[state] = {
            **dict(result),
            "created_at": datetime.utcnow(),
        }
    ...
    @classmethod
    def get_result(cls, state: str) -> dict[str, Any] | None:
        value = cls._results.get(state)
        if value is None:
            return None
        created_at = value.get("created_at")
        if not isinstance(created_at, datetime):

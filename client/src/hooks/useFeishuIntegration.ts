...
import {
  ...
  type FeishuCollaborationOAuthResult,
  type FeishuCollaborationOAuthStart,
  type FeishuChat,
...
export function useFeishuIntegration(enabled = true, options: UseFeishuIntegrationOptions = {}) {
  ...
  const [oauthSession, setOauthSession] = useState<FeishuCollaborationOAuthStart | null>(null)
  const [oauthResult, setOauthResult] = useState<FeishuCollaborationOAuthResult | null>(null)
  ...
  const startOAuth = useCallback(async (data: { default_llm_connection_id?: string | null } = {}) => {
    setOauthLoading(true)
    setOauthResult(null)
    try {
      const next = await ApiService.startFeishuCollaborationOAuth(data)
  ...
  const pollOAuth = useCallback(async (state?: string) => {
    const targetState = state || oauthSession?.state
    if (!targetState) throw new Error('Feishu authorization has not started')
    const result = await ApiService.pollFeishuCollaborationOAuth(targetState)
    setOauthResult(result)
    if (result.status === 'success' && result.installation) {
      setInstallation(result.installation)
      setHealth(await ApiService.getCollaborationInstallationHealth(result.installation.id))
    }
    return result
  }, [oauthSession])

...
import { Trash2, Loader2, Database, Pencil, Upload, FileText, X, Search, Link as LinkIcon, Leaf, Cylinder, Server, HardDrive, Users, Lock, Cloud, ChevronDown, ChevronRight, CheckCircle2, AlertCircle, Network, RefreshCw } from 'lucide-react'
import { ApiService, type ConnectionCreateRequest, type ConnectionType, type Datasource, type DatabricksCatalog, type DatabricksOAuthTokens, type DatabricksWarehouse, type FileType, type SourceResourceProcessing, type SourceResourceType, type SourceSnapshot } from '../services/api'

type DatabricksPair = { catalog: string; schema: string | null }
const pairKey = (p: DatabricksPair) => `${p.catalog}::${p.schema ?? '*'}`
type AddDataType = ConnectionType | 'upload' | 'url' | SourceResourceType
const sourceResourceTypes: SourceResourceType[] = ['pdf', 'web', 'feishu_doc', 'feishu_sheet']
const isSourceResourceType = (value: AddDataType): value is SourceResourceType =>
  sourceResourceTypes.includes(value as SourceResourceType)
import { useDatasources, useCreateDBConnection, useDeleteDBConnection, useUploadMultipleFiles, useUploadFromURL, useCreateSourceResource, useCreateWebSourceResource, useDeleteSourceResource, useSyncSourceResource, useUploadPdfSourceResource } from '../hooks/useDBConnections'
...
export default function DatabasesPage() {
  ...
  const [feishuSourceUrl, setFeishuSourceUrl] = useState('')
  const [sourceResourceProcessing, setSourceResourceProcessing] = useState<Record<string, SourceResourceProcessing>>({})
  const [sourceResourceSnapshots, setSourceResourceSnapshots] = useState<Record<string, SourceSnapshot[]>>({})
  const [sourceDetailsId, setSourceDetailsId] = useState<string | null>(null)
  const sourcePdfInputRef = useRef<HTMLInputElement>(null)

  // Form state for create dialog
  const [selectedType, setSelectedType] = useState<AddDataType>('upload')
  ...
  const uploadPdfSourceMutation = useUploadPdfSourceResource()
  const createWebSourceMutation = useCreateWebSourceResource()
  const createSourceResourceMutation = useCreateSourceResource()
  const syncSourceResourceMutation = useSyncSourceResource()
  const deleteSourceResourceMutation = useDeleteSourceResource()
  const createDialogPending =
    createMutation.isPending ||
    uploadMultipleFilesMutation.isPending ||
    uploadFromURLMutation.isPending ||
    uploadPdfSourceMutation.isPending ||
    createWebSourceMutation.isPending ||
  ...
  const getSourceStatusClasses = (status?: string) => {
    switch (status) {
      case 'ready':
  ...
  const isCreateFormValid = useMemo(() => {
    ...
    if (selectedType === 'pdf') {
      return uploadConnectionName.trim().length > 0 && !!pdfSourceFile
    }

    if (selectedType === 'web') {
      return uploadConnectionName.trim().length > 0 && webSourceUrl.trim().length > 0
    }

    if (selectedType === 'feishu_doc' || selectedType === 'feishu_sheet') {
      return uploadConnectionName.trim().length > 0 && feishuSourceUrl.trim().length > 0
    }
  ...
  }, [selectedType, connectionConfig, databricksStep, selectedPairs, oauthTokens, selectedWarehouseId, uploadConnectionName, pdfSourceFile, webSourceUrl, feishuSourceUrl])
  ...
  const resetForm = () => {
    ...
    resetCSVForm()
    resetUploadForm()
    resetSourceResourceForm()
    resetDatabricksWizard()
  }

  const resetSourceResourceForm = () => {
    setPdfSourceFile(null)
    setWebSourceUrl('')
    setFeishuSourceUrl('')
    setSourceDetailsId(null)
    if (sourcePdfInputRef.current) {
      sourcePdfInputRef.current.value = ''
    }
  }
  ...
  const confirmDelete = async () => {
    ...
    if (connectionToDelete.source_type === 'connection') {
      ...
      deleteSourceResourceMutation.mutate(connectionToDelete.id, {
        onSuccess: () => {
          setDeleteDialogOpen(false)
          setConnectionToDelete(null)
          queryClient.invalidateQueries({ queryKey: ['notebook-connections'] })
  ...
  const handleEditClick = async (datasource: Datasource) => {
    if (datasource.source_type === 'source_resource') {
      if (sourceDetailsId === datasource.id) {
        setSourceDetailsId(null)
        return
      }
      setSourceDetailsId(datasource.id)
      await loadSourceResourceDetails(datasource.id)
      return
    }
  ...
  const handleQuickToggleVisibility = async (datasource: Datasource) => {
    if (datasource.source_type === 'source_resource') {
      showToast.info('Source resource sharing is managed by its workspace visibility policy.')
      return
    }
  ...
  const loadSourceResourceDetails = async (resourceId: string) => {
    try {
      const [processing, snapshots] = await Promise.all([
        ApiService.getSourceResourceProcessing(resourceId),
        ApiService.listSourceResourceSnapshots(resourceId),
      ])
      setSourceResourceProcessing(prev => ({ ...prev, [resourceId]: processing }))
      setSourceResourceSnapshots(prev => ({ ...prev, [resourceId]: snapshots.items }))
    } catch (error: any) {
      showToast.error(`Failed to load source details: ${error.message}`)
    }
  }

  const handleSyncSourceResource = (datasource: Datasource) => {
    syncSourceResourceMutation.mutate(datasource.id, {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ['datasources'] })
        loadSourceResourceDetails(datasource.id)
      },
    })
  }

  const handleCreateSourceResource = () => {
    if (!isCreateFormValid) return

    if (selectedType === 'pdf') {
      if (!pdfSourceFile) return
      uploadPdfSourceMutation.mutate(
        { file: pdfSourceFile, name: uploadConnectionName },
        {
          onSuccess: () => {
            setShowCreateDialog(false)
            resetForm()
    ...
    if (selectedType === 'web') {
      createWebSourceMutation.mutate(
        { name: uploadConnectionName.trim(), source_url: webSourceUrl.trim() },
        {
          onSuccess: () => {
            setShowCreateDialog(false)
            resetForm()
      ...
      createSourceResourceMutation.mutate(
        {
          ...
          source_url: feishuSourceUrl.trim(),
          sync_mode: 'manual',
  ...
  const handleTypeChange = (newType: AddDataType) => {
    resetDatabricksWizard()
    resetSourceResourceForm()
    ...
    <div className="bg-[#0d0d0d] w-full h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar min-h-0">
                  ...
                  <Card className="p-12 text-center bg-[#1a1a1a] border-gray-800">
                    <div className="max-w-md mx-auto">
                      ...
                      <p className="text-gray-400 mb-6">
                        Add a database, CSV/Excel file, PDF, public web page, Feishu Doc/Wiki, or Feishu Sheet.
                      </p>
                    </div>
                  </Card>
                </div>
                ...
                <>
                  ...
                  <div className="max-w-[850px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
                    {displayDatasources.map(datasource => {
                      const timeAgo = formatTimeAgo(datasource.updated_at || datasource.created_at)
                      const isSourceResource = datasource.source_type === 'source_resource'
                      const sourceProcessing = isSourceResource ? sourceResourceProcessing[datasource.id] : undefined
                      const sourceSnapshots = isSourceResource ? sourceResourceSnapshots[datasource.id] || [] : []
                      const latestSnapshot = sourceProcessing?.latest_snapshot || sourceSnapshots[0]
                      return (
                        <Card
                          key={datasource.id}
                          ...
                          <div className="flex items-start justify-between mb-3 gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-3 mb-2 flex-wrap">
                                ...
                                {isSourceResource && (
                                  <span className={`text-xs px-2 py-0.5 rounded shrink-0 ${getSourceStatusClasses(datasource.status)}`}>
                              ...
                              <p className="text-sm text-gray-400 mb-3">
                                {isSourceResource
                                  ? datasource.source_url || `Knowledge source for ${formatDbType(datasource.type)}`
                              ...
                              <p className="text-xs text-gray-500">
                                Updated {timeAgo}
                                {isSourceResource && datasource.latest_snapshot_id ? ` · snapshot ${datasource.latest_snapshot_id.slice(0, 8)}` : ''}
                              </p>
                              {isSourceResource && sourceDetailsId === datasource.id && (
                                <div className="mt-4 rounded-lg border border-gray-800 bg-[#111111] p-4 space-y-3">
                                  <div className="flex items-start justify-between gap-3">
                                    <div>
                                      <p className="text-sm font-medium text-white">Processing evidence</p>
                                      <p className="text-xs text-gray-500">
                                        {latestSnapshot
                                          ? `${latestSnapshot.status} · ${latestSnapshot.parser_version || 'parser pending'}`
                                  ...
                                  {latestSnapshot?.metadata_json?.summary && (
                                    <p className="text-xs text-gray-300 leading-relaxed">
                                      {String(latestSnapshot.metadata_json.summary)}
                                    </p>
                                  )}
                                  {latestSnapshot?.error_json?.message && (
                                    <div className="text-xs text-red-300 bg-red-900/20 border border-red-700/40 rounded-md p-2">
                                      {String(latestSnapshot.error_json.message)}
                                    </div>
                                  )}
                                  {latestSnapshot && (
                                    <div className="text-[11px] text-gray-500 space-y-1">
                                      <div>Hash: <span className="font-mono">{latestSnapshot.content_hash}</span></div>
                                      <div>Raw snapshot: <span className="font-mono break-all">{latestSnapshot.raw_storage_uri}</span></div>
                                    </div>
                                  )}
                                  {sourceSnapshots.length > 1 && (
                                    <p className="text-[11px] text-gray-500">{sourceSnapshots.length} snapshots captured.</p>
                                  )}
                                </div>
                              )}
                            </div>
                            ...
                            <div className="flex gap-2 shrink-0" onClick={(e) => e.stopPropagation()}>
                              {showSharingFeatures && !isSourceResource && canEditDatasource(datasource.created_by) && (
                                <Button
                                  size="sm"
                                  variant="ghost"
                              ...
                              {canEditDatasource(datasource.created_by) && (
                                <Button
                                  ...
                                  onClick={() => handleEditClick(datasource)}
                                  className="text-gray-400 hover:text-white hover:bg-gray-800"
                                  title={isSourceResource ? 'View source details' : 'Edit datasource'}
                                >
                                  {isSourceResource ? <FileText className="w-4 h-4" /> : <Pencil className="w-4 h-4" />}
                                </Button>
                              )}
                              {isSourceResource && (
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  onClick={() => handleSyncSourceResource(datasource)}
                                  disabled={
                                    syncSourceResourceMutation.isPending ||
                                    datasource.type === 'pdf'
                                  }
                                  className="text-gray-400 hover:text-white hover:bg-gray-800 disabled:opacity-40"
                                  title={datasource.type === 'pdf' ? 'PDF refresh requires re-upload' : 'Refresh source'}
                                >
                                  {syncSourceResourceMutation.isPending ? (
                                    <Loader2 className="w-4 h-4 animate-spin" />
                                  ) : (
                              ...
                              {canDeleteDatasource(datasource.created_by) && (
                                <Button
                                  ...
                                  disabled={deleteMutation.isPending || deleteSourceResourceMutation.isPending}
        ...
        <Dialog open={showCreateDialog} onOpenChange={(open) => {
          if (!open && createDialogPending) return
          ...
          <DialogContent className="max-w-4xl bg-[#2a2a2a] border-[#444444] p-0 gap-0">
            <DialogHeader className="px-6 pt-6 pb-4 border-b border-[#444444]">
              <DialogTitle className="text-white text-xl">Add data</DialogTitle>
            </DialogHeader>
              ...
              <div className="w-52 bg-[#1a1a1a] border-r border-[#444444] p-3 overflow-y-auto custom-scrollbar">
                <div className="space-y-1">
                  ...
                  <button
                    ...
                    } ${createDialogPending ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                  ...
                  >
                    <FileText className={`w-5 h-5 flex-shrink-0 ${selectedType === 'pdf' ? 'text-brand-orange' : 'text-gray-400'}`} />
                    <span className="text-sm font-medium">PDF / local file</span>
                  </button>
                  ...
                  >
                    <LinkIcon className={`w-5 h-5 flex-shrink-0 ${selectedType === 'web' ? 'text-brand-orange' : 'text-gray-400'}`} />
                    <span className="text-sm font-medium">Online Web</span>
                  </button>

                  {/* Feishu Doc/Wiki */}
                  <button
                    onClick={() => handleTypeChange('feishu_doc')}
                    disabled={createDialogPending}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md transition-all text-left ${
                      selectedType === 'feishu_doc'
                        ? 'bg-brand-orange/10 text-white border-l-3 border-brand-orange'
                        : 'text-gray-400 hover:text-white hover:bg-[#2a2a2a]'
                    } ${createDialogPending ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    <FileText className={`w-5 h-5 flex-shrink-0 ${selectedType === 'feishu_doc' ? 'text-brand-orange' : 'text-gray-400'}`} />
                    <span className="text-sm font-medium">Feishu Doc/Wiki</span>
                  </button>

                  {/* Feishu Sheet */}
                  <button
                    onClick={() => handleTypeChange('feishu_sheet')}
                    disabled={createDialogPending}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md transition-all text-left ${
                      selectedType === 'feishu_sheet'
                        ? 'bg-brand-orange/10 text-white border-l-3 border-brand-orange'
                        : 'text-gray-400 hover:text-white hover:bg-[#2a2a2a]'
                    } ${createDialogPending ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    <HardDrive className={`w-5 h-5 flex-shrink-0 ${selectedType === 'feishu_sheet' ? 'text-brand-orange' : 'text-gray-400'}`} />
                    <span className="text-sm font-medium">Feishu Sheet</span>
                  </button>
                    ...
                    {selectedType !== 'databricks' && (
                      <div>
                        <Label htmlFor="connection-name" className="text-white">
                          {selectedType === 'upload' || selectedType === 'url' || isSourceResourceType(selectedType) ? 'Data Name' : 'Connection Name'} <span className="text-red-400">*</span>
                        </Label>
                        <Input
                          id="connection-name"
                          value={selectedType === 'upload' || selectedType === 'url' || isSourceResourceType(selectedType) ? uploadConnectionName : connectionConfig.name}
                          onChange={(e) => {
                            if (selectedType === 'upload' || selectedType === 'url' || isSourceResourceType(selectedType)) {
                              setUploadConnectionName(e.target.value)
                            } else {
                              setConnectionConfig(prev => ({ ...prev, name: e.target.value }))
                            }
                          }}
                          placeholder={selectedType === 'upload' || selectedType === 'url' || isSourceResourceType(selectedType) ? 'My Data Source' : 'My Database Connection'}
                          disabled={createDialogPending}
                      ...
                      <div className="space-y-4">
                        {uploadPdfSourceMutation.isPending && (
                          <div className="bg-orange-900/20 border border-brand-orange rounded-lg p-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="text-sm font-medium text-brand-orange">Uploading and parsing PDF…</p>
                                <p className="text-xs text-gray-400 mt-1">The original file is stored as a source snapshot before parsing.</p>
                              </div>
                            </div>
                          </div>
                        )}
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (file && !uploadConnectionName.trim()) {
                        ...
                        <button
                          ...
                          disabled={createDialogPending}
                          className="w-full border-2 border-dashed border-[#555555] hover:border-[#777777] rounded-lg p-8 text-center transition-colors disabled:opacity-50"
                        >
                          <Upload className="w-10 h-10 mx-auto mb-3 text-brand-orange" />
                          <p className="text-white font-medium">{pdfSourceFile ? pdfSourceFile.name : 'Choose a PDF file'}</p>
                          <p className="text-xs text-gray-400 mt-2">
                            MVP parses selectable text, creates a source snapshot, and indexes evidence with page locator metadata.
                          </p>
                        </button>
                      </div>
                    )}
                        ...
                        {createWebSourceMutation.isPending && (
                          <div className="bg-orange-900/20 border border-brand-orange rounded-lg p-4">
                            <div className="flex items-center gap-3">
                              <Loader2 className="w-5 h-5 text-brand-orange animate-spin flex-shrink-0" />
                              <div>
                                <p className="text-sm font-medium text-brand-orange">Fetching and understanding web page…</p>
                                <p className="text-xs text-gray-400 mt-1">Public HTTP/HTTPS pages only; private/local network URLs are blocked.</p>
                              </div>
                            </div>
                          </div>
                        )}
                        <div>
                          <Label htmlFor="web-source-url" className="text-white">Public URL <span className="text-red-400">*</span></Label>
                          <Input
                            id="web-source-url"
                            value={webSourceUrl}
                            onChange={(e) => {
                              setWebSourceUrl(e.target.value)
                              if (!uploadConnectionName.trim()) {
                                try {
                                  const url = new URL(e.target.value)
                                  setUploadConnectionName(url.hostname)
                                } catch {
                          ...
                          <p className="text-xs text-gray-400 mt-1">
                            Byaan stores the raw HTML snapshot, cleans readable text, and blocks loopback/private metadata targets.
                          </p>
                        </div>
                      </div>
                    )}

                    {(selectedType === 'feishu_doc' || selectedType === 'feishu_sheet') && (
                      <div className="space-y-4">
                        <div className="bg-amber-900/20 border border-amber-700/40 rounded-md p-3 text-sm text-amber-200">
                          Feishu OAuth/import is not configured in this Team build yet. This creates a tracked source resource in “Needs authorization” state; sync will not pretend success until production OAuth is configured.
                        </div>
                        <div>
                          <Label htmlFor="feishu-source-url" className="text-white">
                            {selectedType === 'feishu_doc' ? 'Feishu Doc/Wiki URL' : 'Feishu Sheet URL'} <span className="text-red-400">*</span>
                          </Label>
                          <Input
                            id="feishu-source-url"
                            value={feishuSourceUrl}
                            onChange={(e) => setFeishuSourceUrl(e.target.value)}
                            placeholder={selectedType === 'feishu_doc' ? 'https://.../docx/... or https://.../wiki/...' : 'https://.../sheets/...'}
                            disabled={createDialogPending}
                            className="mt-1 bg-[#1a1a1a] border-[#555555] text-white placeholder-[#888888]"
                          />
                          <p className="text-xs text-gray-400 mt-1">
                            The resource will be listed and persisted, but remains blocked on authorization/configuration.
                          </p>
                        </div>
                      </div>
                    )}
                ...
                <div className="border-t border-[#444444] p-6 flex justify-end gap-2">
                  ...
                  >
                    Cancel
	                  ...
	                  <Button
	                    onClick={() => {
	                      if (isSourceResourceType(selectedType)) {
	                        handleCreateSourceResource()
	                      } else if (selectedType === 'upload' || selectedType === 'url') {
	                    ...
	                    disabled={
	                      ...
	                      (isSourceResourceType(selectedType) && !isCreateFormValid) ||
	                      (selectedType !== 'upload' && selectedType !== 'url' && !isSourceResourceType(selectedType) && !isCreateFormValid) ||
	                      createDialogPending
	                    }
	                    className={`${
	                      ((selectedType === 'upload' && uploadConnectionName.trim() && uploadFileType && uploadFiles.length > 0) ||
	                  ...
	                  >
	                    ...
	                    {createDialogPending ? 'Creating...' : 'Add data'}
	                  </Button>
                  </>
                  )}
                </div>
        ...
        <Dialog open={deleteDialogOpen} onOpenChange={(open) => {
          if (!open && (deleteMutation.isPending || deleteSourceResourceMutation.isPending)) return
          setDeleteDialogOpen(open)
        }}>
          <DialogContent className="max-w-md bg-[#2a2a2a] border-[#444444]">
            <DialogHeader>
              <DialogTitle className="text-white">Delete data source?</DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 mt-6">
                ...
                >
                  ...
                  {deleteMutation.isPending || deleteSourceResourceMutation.isPending ? 'Deleting...' : 'Delete'}
                </Button>
              </div>
            </div>
          </DialogContent>

...
export function DemoBadge() {
  return (
    <span className="inline-flex items-center rounded border border-brand-orange/30 bg-brand-orange/10 px-2 py-0.5 text-[11px] font-medium uppercase tracking-normal text-brand-orange">
      Demo Mock
    </span>
  )
}
...
export function PermissionState({ action }: { action?: ReactNode }) {
  return (
    <div className="flex min-h-[260px] flex-col items-center justify-center rounded-lg border border-amber-500/20 bg-amber-500/5 p-8 text-center">
      ...
      <p className="mt-2 max-w-md text-sm text-amber-100/70">This mock state shows how Data Models handles a user without model governance access.</p>
      {action && <div className="mt-4">{action}</div>}
    </div>
  )
}

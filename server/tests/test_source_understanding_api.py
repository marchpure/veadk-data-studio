...
async def _create_connection_dataset(test_session, tenant_id, user_id, schema=SALES_SCHEMA):
    connection = Connection(
        ...
        schema_cache=json.dumps(deepcopy(schema)),
        is_public=True,
    )
...
async def test_source_understanding_detects_drift_and_marks_previous_verified_candidates_stale(
    ...
    await test_session.refresh(refreshed_connection)
    ...
    schema_response = await test_client.get(f"/api/datasources/{dataset.id}/schema")
    assert schema_response.status_code == 200
    assert any(
        column["name"] == "CHANNEL_ID"
        for column in schema_response.json()["data"]["schema"]["ORDERS"]["columns"]
    )

...
def _table_reference(table: str, schema: str | None = None) -> str:
...
class SemanticModelService:
    ...
    @staticmethod
    async def list_published_models(session: AsyncSession, tenant_id: UUID) -> list[dict[str, Any]]:
        result = await session.execute(
            select(SemanticModel)
            .where(SemanticModel.tenant_id == tenant_id, SemanticModel.published_version != "v0")
            .order_by(SemanticModel.updated_at.desc())
            .options(
                selectinload(SemanticModel.entities).selectinload("*"),
                selectinload(SemanticModel.relationships),
        ...
        for model in result.scalars().unique().all():
            published = await SemanticModelService.published_payload(session, model)
            if published is not None:
                payloads.append(published)
        return payloads
    ...
    @staticmethod
    def _definition_payload(model: SemanticModel) -> dict[str, Any]:
        return {
            "name": model.name,
            "domain": model.domain,
            "owner": model.owner,
            ...
            "calculatedFields": _json_load(model.review_json, {}).get("calculatedFields") or [],
        }

    @staticmethod
    def definition_hash(model: SemanticModel) -> str:
        return _json_hash(SemanticModelService._definition_payload(model))

    @staticmethod
    def _invalidate_review_for_definition_change(model: SemanticModel) -> None:
        review = _json_load(model.review_json, {})
        for key in (
            "reviewed",
            "reviewedAt",
            ...
            review.pop(key, None)
        readiness_detail = SemanticModelService._not_validated_detail("Definition changed. Run Validate before review and publish.")
        review["readinessDetail"] = readiness_detail
        model.review_json = _json_dump(review)
        model.readiness = 0
        model.readiness_level = "blocked"
        for metric in model.metrics:
            if metric.certification == "certified":
                metric.certification = "reviewed"

    @staticmethod
    def _record_review_approval(model: SemanticModel, user_id: UUID | None) -> None:
        review = _json_load(model.review_json, {})
        review["opened"] = True
        review["reviewed"] = True
        review["reviewedAt"] = datetime.utcnow().isoformat()
        review["reviewedBy"] = str(user_id) if user_id else None
        review["reviewedRevision"] = model.revision
        review["reviewedDraftRevision"] = model.draft_revision
        review["reviewedDefinitionHash"] = SemanticModelService.definition_hash(model)
        model.review_json = _json_dump(review)

    @staticmethod
    def _current_validation_summary(model: SemanticModel) -> dict[str, Any] | None:
        review = _json_load(model.review_json, {})
        validation = review.get("validationSummary") if isinstance(review.get("validationSummary"), dict) else None
        if not validation:
            return None
        if validation.get("status") != "passed":
            return None
        if validation.get("draftRevision") != model.draft_revision:
            return None
        if validation.get("definitionHash") != SemanticModelService.definition_hash(model):
            return None
        if not validation.get("queryChecks"):
            return None
        return validation

    @staticmethod
    def _current_readiness_detail(model: SemanticModel) -> dict[str, Any]:
        review = _json_load(model.review_json, {})
        validation = SemanticModelService._current_validation_summary(model)
        if validation is None:
            return SemanticModelService._not_validated_detail("Current Semantic Model definition has not passed validation.")
        detail = review.get("readinessDetail")
        if isinstance(detail, dict):
            return detail
        return SemanticModelService._not_validated_detail("Current Semantic Model definition has no readiness detail.")

    @staticmethod
    def _can_certify_metrics(model: SemanticModel) -> bool:
        if SemanticModelService._current_validation_summary(model) is None:
            return False
        review = _json_load(model.review_json, {})
        return (
            review.get("reviewed") is True
            and review.get("reviewedDraftRevision") == model.draft_revision
            and review.get("reviewedDefinitionHash") == SemanticModelService.definition_hash(model)
        )

    @staticmethod
    def _not_validated_detail(reason: str) -> dict[str, Any]:
        return {
            "score": 0,
            "level": "blocked",
            "components": [
                {"id": "validation", "name": "Validation evidence", "score": 0, "status": "blocked"},
                {"id": "query", "name": "Query correctness", "score": 0, "status": "blocked"},
            ],
            "reliableQuestions": [],
            "unreliableQuestions": ["Semantic questions are not reliable until validation query evidence passes."],
            "blockers": [reason],
            "warnings": [],
        }
    ...
    async def update_model(
        ...
        review_patch = payload.get("review") if isinstance(payload.get("review"), dict) else None
        if review_patch is not None:
            review = _json_load(model.review_json, {})
            sanitized_review_patch = {
                key: value
                for key, value in review_patch.items()
                if key in {"opened", "publishNotes"}
            }
            model.review_json = _json_dump({**review, **sanitized_review_patch})
        ...
        if definition_changed:
            ...
            SemanticModelService._invalidate_review_for_definition_change(model)
        elif review_patch and review_patch.get("reviewed") is True:
            review = _json_load(model.review_json, {})
            validation = review.get("validationSummary") if isinstance(review.get("validationSummary"), dict) else {}
            if validation.get("status") != "passed":
                raise ValueError("Semantic Model must pass validation before review can be approved.")
            if validation.get("draftRevision") != model.draft_revision:
                raise ValueError("Semantic Model validation is stale. Run Validate before approving review.")
            if validation.get("definitionHash") != SemanticModelService.definition_hash(model):
                raise ValueError("Semantic Model validation does not match the current definition.")
            SemanticModelService._record_review_approval(model, user_id)
    ...
    async def _replace_metrics(session: AsyncSession, model: SemanticModel, items: list[dict[str, Any]]) -> None:
        ...
        for index, item in enumerate(items):
            ...
            requested_certification = str(item.get("certification") or "draft")
            if requested_certification == "certified":
                requested_certification = "reviewed"
            metric.certification = requested_certification
    ...
    def _readiness_detail(model: SemanticModel) -> dict[str, Any]:
        ...
        query_checks = validation.get("queryChecks") or []
        query = 90 if query_checks and all(check.get("status") == "passed" for check in query_checks) else 0
        ...
        if query < 85:
            blockers.append("Metric query validation evidence is missing or failed.")
    ...
    @staticmethod
    def _validation_blockers(model: SemanticModel, query_checks: list[dict[str, Any]]) -> list[str]:
        blockers = []
        blocked_relationships = [
            rel for rel in model.relationships if rel.validation_status == "blocked" and rel.status != "rejected"
        ]
        ...
        if blocked_relationships:
            blockers.append("Blocked relationships must be fixed or rejected.")
        if not query_checks:
            blockers.append("No metric query validation evidence was produced.")
        failed_checks = [check for check in query_checks if check.get("status") != "passed"]
        if failed_checks:
            blockers.append("One or more metric validation queries failed.")
        return blockers

    @staticmethod
    def _readiness_detail_from_checks(
        model: SemanticModel,
        ...
        governance = 85 if all(metric.certification != "draft" for metric in model.metrics) else 68
        evidence = 80 if any(_json_load(metric.lineage_json, []) for metric in model.metrics) else 50
        score = 0 if blockers else round(structural * 0.2 + semantic * 0.25 + query_score * 0.25 + governance * 0.15 + evidence * 0.15)
        warnings = []
        if any(metric.certification == "draft" for metric in model.metrics):
            warnings.append("Some metrics are still draft certified.")
        if query_checks and any(check.get("returnedCount") in (None, 0) for check in query_checks):
            warnings.append("Some validation queries returned no rows; confirm metric sample coverage before publish.")
        return {
            "score": score,
            "level": "blocked" if blockers else "ready" if score >= 85 else "warning",
            "components": [
                {
                    ...
                    "score": structural,
                    "status": "ready" if structural >= 85 else "blocked",
                },
                {
                    "id": "semantic",
                ...
                },
                {
                    ...
                    "score": query_score,
                    "status": "ready" if query_score >= 85 else "blocked",
                },
                {
                    "id": "governance",
                    "name": "Governance",
            ...
            "reliableQuestions": [] if blockers else [f"What is {metric.business_name} by available dimensions?" for metric in model.metrics],
            "unreliableQuestions": ["Semantic questions are not reliable until validation query evidence passes."] if blockers else [],
            "blockers": blockers,
            "warnings": warnings,
        }

    @staticmethod
    async def _run_validation_query_checks(
        session: AsyncSession,
        tenant_id: UUID,
        model: SemanticModel,
    ) -> list[dict[str, Any]]:
        ...
        for metric in model.metrics:
            ...
            dimensions = _json_load(metric.dimensions_json, [])
            if dimensions:
                dimension = SemanticModelService._dimension_by_slug_or_name(model, str(dimensions[0]))
            try:
                ...
                result_rows = result.get("result") if isinstance(result.get("result"), list) else []
                ...
                check = {
                    ...
                    "sql": safe_sql,
                    "returnedCount": int(returned_count or 0),
                    "error": result.get("error") or "",
                }
                metric.compiled_sql = safe_sql if not failed else ""
                metric.validation_status = "valid" if not failed else "blocked"
                metric.preview_json = _json_dump(
                    {
                        "currentValue": "Validated" if not failed else "Validation failed",
                        "trend": "",
                        "breakdown": [],
                        "explanation": metric.definition,
                        "sql": safe_sql if not failed else "",
                        "validation": "Validation query passed." if not failed else str(result.get("error") or "Validation query failed."),
                    }
                )
                checks.append(check)
            except Exception as error:
                metric.compiled_sql = ""
                metric.validation_status = "blocked"
                metric.preview_json = _json_dump(
                    {
    ...
    async def validate_model(session: AsyncSession, tenant_id: UUID, slug: str, user_id: UUID | None) -> dict[str, Any] | None:
        ...
        definition_hash = SemanticModelService.definition_hash(model)
        query_checks = await SemanticModelService._run_validation_query_checks(session, tenant_id, model)
        validation_blockers = SemanticModelService._validation_blockers(model, query_checks)
        query_score = 90 if query_checks and not any(check["status"] != "passed" for check in query_checks) else 0
        detail = SemanticModelService._readiness_detail_from_checks(model, query_checks, validation_blockers, query_score)
        ...
        review = _json_load(model.review_json, {})
        for key in ("reviewed", "reviewedAt", "reviewedBy", "reviewedRevision", "reviewedDraftRevision", "reviewedDefinitionHash"):
            review.pop(key, None)
    ...
    async def publish_model(session: AsyncSession, tenant_id: UUID, slug: str, user_id: UUID | None) -> dict[str, Any] | None:
        ...
        validation = SemanticModelService._current_validation_summary(model)
        if validation is None:
            raise ValueError("Semantic Model cannot be published until the current definition passes validation.")
        detail = SemanticModelService._current_readiness_detail(model)
        if detail["blockers"]:
        ...
        if review.get("reviewed") is not True:
            raise ValueError("Semantic Model cannot be published until human review is approved.")
        if review.get("reviewedDraftRevision") != model.draft_revision:
            raise ValueError("Semantic Model review is stale. Approve review again after validation.")
        if review.get("reviewedDefinitionHash") != SemanticModelService.definition_hash(model):
            raise ValueError("Semantic Model review does not match the current definition.")
        ...
        model.readiness_level = str(detail["level"])
        for metric in model.metrics:
            metric.certification = "certified"
        mcp = _json_load(model.mcp_json, {})
    ...
    def _compile_metric_sql(
        ...
        table_ref = _table_reference(entity.table_name, entity_profile.get("schema"))
    ...
    def _compile_payload_metric_sql(
        ...
        if entity is None:
            raise ValueError("Published Semantic Model has no entity to query.")
        table_ref = _table_reference(str(entity.get("table") or ""), entity.get("schema") or (entity.get("profile") or {}).get("schema"))
        select_parts: list[str] = []
        group_parts: list[str] = []
        join_clause = ""
        if dimension is not None:
    ...
    def _join_clause_for_dimension(
        ...
        for relationship in model.relationships:
            if relationship.from_entity == base_entity.slug and relationship.to_entity == target.slug:
                return SemanticModelService._relationship_join_clause(relationship, target)
            if relationship.from_entity == target.slug and relationship.to_entity == base_entity.slug:
                return SemanticModelService._relationship_join_clause(relationship, target, reverse=True)
        return ""
    ...
    def _relationship_join_clause(
        ...
        *,
        ...
        target_ref = _table_reference(target.table_name, target_profile.get("schema"))
    ...
    def _payload_join_clause_for_dimension(
        ...
        for relationship in model.get("relationships") or []:
            if relationship.get("fromEntity") == base_slug and relationship.get("toEntity") == target_slug:
                return SemanticModelService._payload_relationship_join_clause(relationship, target)
            if relationship.get("fromEntity") == target_slug and relationship.get("toEntity") == base_slug:
                return SemanticModelService._payload_relationship_join_clause(relationship, target, reverse=True)
        return ""
    ...
    def _payload_relationship_join_clause(
        ...
        *,
        ...
        if not predicates:
            return ""
        target_ref = _table_reference(str(target.get("table") or ""), target.get("schema") or (target.get("profile") or {}).get("schema"))
    ...
    @staticmethod
    async def published_payload(
        session: AsyncSession,
        model: SemanticModel,
    ) -> dict[str, Any] | None:
        if model.published_version == "v0":
            return None
        return await SemanticModelService._published_snapshot_payload(session, model)

    @staticmethod
    async def load_published_model_payload(
        session: AsyncSession,
        tenant_id: UUID,
        slug: str,
    ) -> dict[str, Any] | None:
        ...
        if model is None:
            return None
        return await SemanticModelService.published_payload(session, model)
    ...
    async def run_query_metric(
        ...
        raise RuntimeError("Published Semantic Model version snapshot was not found.")
    ...
    async def _run_published_payload_query_metric(
        ...
        payload = {
            ...
            "result": result.get("result", result) if not result.get("error") else None,

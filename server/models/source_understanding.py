...
class SourceSkillCandidate(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            f"candidate_type IN {SOURCE_SKILL_CANDIDATE_TYPES}", name="ck_source_skill_candidates_type"
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            f"review_status IN {SOURCE_SKILL_REVIEW_STATUSES}", name="ck_source_skill_candidates_review_status"
        ),
    )

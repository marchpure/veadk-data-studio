...
- The backend processing payload now returns a typed `status`, `stage`, and structured `steps` contract. Step ids are limited to `capture`, `parse`, `detect_tables`, `normalize_dataset`, `index_context`, `generate_semantic_suggestions`, and `ready`; step status is limited to `pending`, `running`, `succeeded`, `skipped`, or `failed`, so import cards and Source Detail consume the same Source processing contract instead of each view guessing readiness from a single stage.
...
- Source Detail now renders the backend `SourceOverviewItem` modeling handoff directly for every source type: status, mode, profile-loading gate, reason, next action, and evidence summary. This keeps detail pages aligned with the Data Modeling picker instead of duplicating family heuristics in the page.

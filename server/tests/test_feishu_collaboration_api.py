...
from server.services.source_connectors import FeishuAdminConfigService
...
async def test_feishu_config_api_uses_admin_app_config_when_public_payload_has_no_secret(
    test_client,
    test_session,
    monkeypatch,
):
    ...
    async def fake_probe(self):
        ...
        return {
            "ok": True,
            "bot": {"open_id": "ou_bot", "tenant_key": "tenant-managed"},
            "bot_external_id": "ou_bot",
            "external_tenant_id": "tenant-managed",
            "external_tenant_name": "Managed Tenant",
        }
    ...
    response = await test_client.post(
        "/api/collaboration/installations/feishu",
        json={"connection_mode": "websocket"},
    )

    assert response.status_code == 200
    data = response.json()["data"]
    assert data["app_id"] == "cli_managed"
    assert "managed-secret" not in str(data)
    assert probes == [("cli_managed", "managed-secret")]


async def test_feishu_collaboration_oauth_uses_existing_callback_and_creates_installation(
    test_client,
    test_session,
    monkeypatch,
):
    ...
    async def fake_exchange_code(self, *, config, code):
        ...
        assert config["app_secret"] == "oauth-secret"
        return {
    ...
    async def fake_user_info(self, access_token):
        assert access_token == "user-access-token"
        return {"open_id": "ou_authorized_user", "union_id": "on_union", "name": "Authorized User"}

    async def fake_probe(self):
        assert self.app_id == "cli_collab_oauth"
        assert self.app_secret == "oauth-secret"
        return {
            "ok": True,
            "bot": {"open_id": "ou_bot", "tenant_key": "tenant-collab-oauth"},
            "bot_external_id": "ou_bot",
            "external_tenant_id": "tenant-collab-oauth",
            "external_tenant_name": "Collab OAuth Tenant",
        }

    from server.services.source_connectors import FeishuConnectorAdapter

    monkeypatch.setattr(FeishuConnectorAdapter, "_exchange_code", fake_exchange_code)
    monkeypatch.setattr(FeishuConnectorAdapter, "_get_user_info", fake_user_info)
    monkeypatch.setattr(FeishuApiClient, "probe", fake_probe)

    start = await test_client.post(
        "/api/collaboration/installations/feishu/oauth/start",
        json={},
    )
    assert start.status_code == 200
    started = start.json()["data"]
    assert "open-apis/authen/v1/authorize" in started["authorization_url"]
    assert "app_id=cli_collab_oauth" in started["authorization_url"]
    assert "oauth-secret" not in str(started)
    assert started["redirect_uri"] == "http://127.0.0.1:8080/api/source-connections/feishu/oauth/callback"
    assert started["qr_payload"] == started["authorization_url"]

    pending = await test_client.get(f"/api/collaboration/installations/feishu/oauth/result?state={started['state']}")
    assert pending.status_code == 200
    assert pending.json()["data"]["status"] == "pending"

    callback = await test_client.get(
        "/api/source-connections/feishu/oauth/callback",
        params={"code": "oauth-code", "state": started["state"]},
    )
    assert callback.status_code == 200
    callback_data = callback.json()["data"]
    assert callback_data["status"] == "success"
    assert callback_data["installation"]["app_id"] == "cli_collab_oauth"
    assert "oauth-secret" not in str(callback_data)
    assert callback_data["external_identity"]["external_user_id"] == "ou_authorized_user"

    result = await test_client.get(f"/api/collaboration/installations/feishu/oauth/result?state={started['state']}")
    assert result.status_code == 200
    assert result.json()["data"]["status"] == "success"
    assert result.json()["data"]["installation"]["external_tenant_id"] == "tenant-collab-oauth"

    identity = (await test_session.execute(select(ExternalIdentity))).scalar_one()
    assert identity.external_user_id == "ou_authorized_user"
    assert identity.union_id == "on_union"
    assert identity.byaan_user_id == tenant.owner_id

...
class FeishuApiClient:
    ...
    async def list_chats(self, *, page_token: str | None = None, page_size: int = 50) -> dict[str, Any]:
        token = await self.tenant_access_token()
        params = f"page_size={max(1, min(page_size, 100))}"
        if page_token:
            params += f"&page_token={page_token}"
        data = await self._request("GET", f"/im/v1/chats?{params}", token=token)

    ...
    <main className="min-h-0 flex-1 overflow-y-auto bg-[#171717] p-3 custom-scrollbar">
      <div className="mx-auto grid max-w-[1500px] gap-4">
        <div className="rounded-md border border-brand-orange/25 bg-brand-orange/10 px-4 py-3 text-sm text-[#f4d0bd]">
          Explore uses the published Semantic Model through query_metric. Draft changes must be validated and published before MCP consumers see them.
        </div>
            ...
            <div className="min-w-0 space-y-4">
              ...
              <div className="rounded-md border border-[#2a2a2a] bg-[#181818] p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <h2 className="text-sm font-semibold text-white">{metric?.businessName ?? 'No metric'} by {dimension?.name ?? 'No dimension'}</h2>
...
function EmptyQueryResult({ published }: { published: boolean }) {
  return (
    <div className="mt-5 flex min-h-[300px] flex-col items-center justify-center rounded-md border border-dashed border-[#333] bg-[#151515] p-6 text-center">
      <DatabaseZap className="mb-3 h-6 w-6 text-[#777]" />
      <h3 className="text-sm font-semibold text-white">{published ? 'Run query_metric to load semantic results' : 'Publish before querying'}</h3>
      <p className="mt-2 max-w-md text-sm text-[#9a9a9a]">
        {published

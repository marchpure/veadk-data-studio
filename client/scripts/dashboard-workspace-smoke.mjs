...
async function runEditReviewScene(page, fixtures, viewport) {
  ...
  await page.getByText('Tile added').waitFor()
  await page.getByLabel('Tile title').fill(`Added Tile ${viewport}`)
  await page.getByLabel('Tile business question').fill(`Which added tile survives viewport ${viewport}?`)
  await page.getByLabel('Tile encoding JSON').fill(JSON.stringify({ value: 'revenue', label: 'region', added: true }, null, 2))
  await page.getByRole('button', { name: 'Save Tile' }).click()
  await page.getByText('Tile inspector saved').waitFor()
  await page.getByRole('button', { name: `Select tile Added Tile ${viewport}` }).waitFor()
  await page.getByLabel(`Move Added Tile ${viewport} up`).click()
  await page.getByText('Tile order updated').waitFor()
  await page.getByRole('button', { name: 'Remove Tile' }).click()
  await page.getByText('Tile removed').waitFor()
  await page.getByRole('button', { name: `Select tile Added Tile ${viewport}` }).waitFor({ state: 'detached' })

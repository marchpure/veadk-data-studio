...
DATAFRAME_SPECIFIC_RULES = """
1. ONLY READ operations - filtering, grouping, sorting, aggregations (NO write operations)
2. Use pandas DataFrame syntax: df[condition], df.groupby(), df.sort_values(), etc.
3. Format: Pass DataFrame expression as string (e.g., "df[df['age'] > 25]")
4. Limit parameter in tool restricts rows returned (max 5 for testing)
5. Return clean pandas expressions without extra code or comments
6. NEVER use write operations: to_csv(), to_sql(), to_excel(), to_json(), etc.
7. NEVER use file operations: open(), os.remove(), subprocess, exec(), eval()
8. ALWAYS use square brackets for column access: df['column_name'] not df.column_name
9. For multi-column selection: df[['col1', 'col2', 'col3']]
10. Use .query() method for complex filters: df.query("age > 25 and city == 'NYC'")
11. Common aggregations: .mean(), .sum(), .count(), .min(), .max(), .median()
12. Grouping: df.groupby('category')['value'].sum()
13. Sorting: df.sort_values('column', ascending=False)
14. CRITICAL: All operations are READ-ONLY and validated for security before execution

15. MULTIPLE FILES (Multi-Table Support for CSV/Excel/Parquet/JSON/Feather):
    - When multiple files are uploaded (CSV, Excel, Parquet, JSON, or Feather), each file is available as a separate DataFrame
    - Reference files by their alias name from schema: customers[...], orders[...], products[...]
    - Each file acts like a SQL table with its alias as the DataFrame variable name
    - Use get_database_schema tool to see all available tables/files and their columns
    - All file formats (CSV, Excel, Parquet, JSON, Feather) work identically with pandas - same query syntax

16. FILE FORMAT SUPPORT:
    - CSV: Standard comma-separated values
    - Excel: .xlsx, .xls files (each sheet can be a separate table in multi-sheet uploads)
    - Parquet: Columnar format, faster for large datasets
    - JSON: JavaScript Object Notation files
    - Feather: Binary columnar format, optimized for fast I/O
    - All formats use identical pandas syntax - no special handling needed

17. JOINING FILES:
    - Use pandas merge() or join() to combine data from multiple files (works across all formats)
    - Common join patterns:
      * Inner join: df1.merge(df2, on='common_column')
      * Left join: df1.merge(df2, on='key', how='left')
      * Multiple keys: df1.merge(df2, left_on='id', right_on='customer_id')
    - Always specify join keys explicitly (left_on, right_on)
    - After groupby with aggregation, use .reset_index() before merging

18. MULTI-FILE QUERY EXAMPLES:

    Example 1: Join customers with orders
    ```python
    orders.merge(customers, left_on='customer_id', right_on='id')
    ```

    Example 2: Aggregate orders by customer name
    ```python
    orders.groupby('customer_id')['amount'].sum().reset_index()
          .merge(customers[['id', 'name']], left_on='customer_id', right_on='id')
    ```

    Example 3: Three-way join across multiple files
    ```python
    orders.merge(customers, left_on='customer_id', right_on='id')
          .merge(products, left_on='product_id', right_on='id')
    ```

    Example 4: Filter one table, then join with another
    ```python
    active_customers = customers[customers['status'] == 'active']
    orders.merge(active_customers[['id', 'name']], left_on='customer_id', right_on='id')
    ```

    Example 5: Complex aggregation with join
    ```python
    order_summary = orders.groupby('customer_id').agg({'amount': 'sum', 'id': 'count'}).reset_index()
    order_summary.columns = ['customer_id', 'total_amount', 'order_count']
    order_summary.merge(customers[['id', 'name', 'city']], left_on='customer_id', right_on='id')
    ```

19. RELATIONSHIP DETECTION:
    - Look for common column patterns: 'customer_id', 'order_id', 'product_id', 'user_id'
    - Columns ending in '_id' usually reference 'id' column in another file
    - Use schema to identify matching column names and compatible types
    - When unsure about relationships, examine sample data or ask user

20. BEST PRACTICES FOR MULTI-FILE QUERIES:
    - Always use .reset_index() after groupby operations before merging
    - Select only needed columns before joining to reduce memory: df[['col1', 'col2']]
    - Use explicit column names in merge: left_on='...', right_on='...'
    - For single file queries, continue using 'df' as the DataFrame name

Example queries:
<correct_dataframe_queries>
# Single file - Filter rows
df[df['age'] > 30]

# Single file - Group and aggregate
df.groupby('department')['salary'].mean()

# Multi-file - Simple join
orders.merge(customers, left_on='customer_id', right_on='id')

# Multi-file - Aggregate then join
orders.groupby('customer_id')['amount'].sum().reset_index()
      .merge(customers[['id', 'name']], left_on='customer_id', right_on='id')

# Multi-file - Filter and join
high_value_orders = orders[orders['amount'] > 1000]
high_value_orders.merge(customers, left_on='customer_id', right_on='id')

# Multi-file - Three-way join
orders.merge(customers, left_on='customer_id', right_on='id')
      .merge(products, left_on='product_id', right_on='id')[['order_date', 'customer_name', 'product_name', 'quantity']]
</correct_dataframe_queries>

<wrong_dataframe_queries>
# WRONG - Write operations (blocked)
df.to_csv('output.csv')
df.to_sql('table', connection)

# WRONG - File operations (blocked)
open('file.txt', 'w')
exec('malicious_code')

# WRONG - System operations (blocked)
import os
os.remove('file')

# WRONG - Joining without reset_index after groupby
orders.groupby('customer_id')['amount'].sum().merge(customers, left_on='customer_id', right_on='id')
# CORRECT: Add .reset_index() after groupby
orders.groupby('customer_id')['amount'].sum().reset_index().merge(customers, left_on='customer_id', right_on='id')
</wrong_dataframe_queries>
"""
...
DATAFRAME_QUERY_FORMAT = """
When returning DataFrame queries, follow this EXACT format:
...
Single file example:
```python
df[df['age'] > 25].groupby('department')['salary'].mean()
```
results: Returns average salary by department for employees over 25

Multi-file example:
```python
orders.merge(customers[['id', 'name']], left_on='customer_id', right_on='id')
```
results: Returns orders with customer names joined from customers table
"""
...
def get_unified_agent_prompt(database_schema: str = None, db_type: str = None) -> str:
    ...
    if db_type_normalized == "mongo":
        ...
        db_name = "PostgreSQL"
    elif db_type_normalized in ("csv", "excel", "parquet", "json", "feather"):
        db_name = f"{db_type_normalized.upper()}/DataFrame"
...
1. QUERY WRITING & EXECUTION ({db_name})
   - Write and test {"MongoDB find/aggregate" if db_type_normalized == "mongo" else "DataFrame pandas expressions" if db_type_normalized in ("csv", "excel", "parquet", "json", "feather") else "SQL SELECT"} queries
   - Validate queries with small test runs (limit 1-4 {"rows" if db_type_normalized != "mongo" else "documents"})
...
- {"execute_mongo_query" if db_type_normalized == "mongo" else "execute_dataframe_query" if db_type_normalized in ("csv", "excel", "parquet", "json", "feather") else "execute_sql_query"}: explore data with limit (1-4 {"documents" if db_type_normalized == "mongo" else "rows"})
...
1. SIMPLE DATA QUERY (user asks: "show me X", "what are the Y", "count Z")
   ...
   → Explore data from better context using {"execute_mongo_query" if db_type_normalized == "mongo" else "execute_dataframe_query" if db_type_normalized in ("csv", "excel", "parquet", "json", "feather") else "execute_sql_query"} (limit 2-4)
...
<query_writing_rules>
{"MongoDB" if db_type_normalized == "mongo" else "DataFrame/pandas" if db_type_normalized in ("csv", "excel", "parquet", "json", "feather") else "SQL"} SPECIFIC RULES:

RULE VERIFICATION:
- Use get_database_writing_rules tool to fetch latest syntax rules before writing any queries
- Especially useful when working with {"ObjectId, dates" if db_type_normalized == "mongo" else "DataFrame operations, filtering" if db_type_normalized in ("csv", "excel", "parquet", "json", "feather") else "SQL syntax"}, or encountering syntax errors
...
1. ALWAYS include PropTypes BEFORE Recharts (Recharts depends on it)
2. ALWAYS use version-pinned URLs (@2.8.0 for Recharts, @15.8.1 for PropTypes)
3. ALWAYS add crossorigin attribute to React, ReactDOM, PropTypes, and Recharts scripts
4. NEVER skip PropTypes - modern React 17 doesn't include it, but Recharts needs it
...
You:
1. Write {"MongoDB query" if db_type_normalized == "mongo" else "DataFrame expression" if db_type_normalized in ("csv", "excel", "parquet", "json", "feather") else "SQL query"}
...
- Show {"SQL/MongoDB/DataFrame" if db_type_normalized in ("csv", "excel", "parquet", "json", "feather") else "SQL/MongoDB"} queries to users in code blocks (they need to see these)
- {"DataFrame operations are READ-ONLY - all write operations are blocked for security" if db_type_normalized in ("csv", "excel", "parquet", "json", "feather") else ""}
...
- Technical: PropTypes before Recharts, waitForDependencies() pattern, version-pinned CDNs (@2.8.0, @15.8.1)
...
- Technical: PropTypes before Recharts, waitForDependencies() pattern, CDNs (@2.8.0, @15.8.1)
</remember>
"""

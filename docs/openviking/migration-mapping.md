...
| W6.1 owner-scoped ResourceRef boundary | `server/services/openviking_service.py`, `server/routers/openviking.py` | Preserve tenant/owner profile isolation, signed opaque references, resolve/read adapters, recursive redaction, stable upstream errors, and two-stage upload. | 23 backend tests plus W6.1 hosted resolve/read/restart/revoke evidence |
| Data Workshop Connection source discovery | `features/openviking/api.ts`, `/api/datasources`, `/connection-resource` | Use the unified datasource collection, filter to `source_resource`, and resolve the selected record server-side instead of accepting browser-supplied documents. | Frontend adapter test and completed hosted Connection Resource import task |
| W6.1 browser acceptance | `client/scripts/openviking-live-e2e.mjs`, `docs/openviking/w61-browser-evidence.json` | Exercise the integrated W6.1 runtime without exposing provider credentials to the browser. | Real resource preview, retrieval, Tasks, Watches, Skill Context, refresh, health, and three responsive screenshots |
